...
function loadLiveEventsData() {
    ...
    fetch('./events_deep_data.json')
        ...
        .then(data => {
            ...
            // గ్రూపింగ్ చేయడానికి విభాగాల హెడర్స్ ట్రాక్ చేస్తాం
            let currentSection = "";

            data.forEach((event, index) => {
                // సెక్షన్ హెడర్ డిస్ప్లే (ఉదాహరణకు ENDS TODAY, ENDS TOMORROW)
                if(event.section && event.section !== currentSection) {
                    currentSection = event.section;
                    const secHeader = document.createElement('div');
                    secHeader.style.cssText = "color: #8E8E93; font-size: 12px; font-weight: 700; margin-top: 10px; border-bottom: 1px solid #222; padding-bottom: 4px; letter-spacing: 0.5px;";
                    secHeader.innerText = currentSection;
                    grid.appendChild(secHeader);
                }

                const card = document.createElement('div');
                card.className = "pogo-event-card";
                
                card.innerHTML = `
                    <div class="pogo-card-body">
                        <img src="${event.imageUrl || 'https://images.gameinfo.io/item/128/pokeball.png'}" class="pogo-card-img">
            ...
            // రియల్-టైమ్ కౌంట్‌డౌన్ అప్‌డేట్ రన్ చేయడానికి ఇంటర్వెల్ సెట్
            timerInterval = setInterval(() => {
                data.forEach((event, index) => {
                    ...
                    if (distance < 0) {
                        ...
                        timerElement.innerText = `${days} days ${hours} hours ${minutes} min`;
                        timerElement.style.color = "#007AFF"; // బ్లూ కలర్ హైలైట్ టైమర్
                    }
                });
            }, 1000);
        })
        .catch(() => {
            if(grid) {
                grid.innerHTML = `<div style="color:#ff3e3e; font-size:12px; text-align:center; padding:10px;">⚠ Live Events Data is syncing via GitHub Actions. Please trigger workflow or check back soon!</div>`;
            }
        });
}
