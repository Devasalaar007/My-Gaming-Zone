...
function loadLiveEventsData() {
    ...
    // బ్రౌజర్ క్యాచీ ని క్లియర్ చేసి ఎప్పటికప్పుడు కొత్త ఫైల్ లోడ్ అవ్వడానికి టైమ్‌స్టాంప్ యాడ్ చేసాను
    fetch(`./events_deep_data.json?v=${new Date().getTime()}`)
        .then(res => {
        ...
        .then(data => {
            ...
            data.forEach((event, index) => {
                ...
                // లీక్‌డక్ ఇమేజ్ లోడ్ అవ్వకపోతే ఆటోమేటిక్‌గా అందమైన పోకేబాల్ ఇమేజ్ సెట్ అవుతుంది
                const displayImg = event.imageUrl ? event.imageUrl : 'https://images.gameinfo.io/item/128/pokeball.png';
                ...
                card.innerHTML = `
                    <div class="pogo-event-card" onclick="openEventDetails(${index})" style="cursor: pointer;">
                        <div class="pogo-card-body">
                            <img src="${displayImg}" 
                                 class="pogo-card-img" 
                                 onerror="this.onerror=null; this.src='https://images.gameinfo.io/item/128/pokeball.png';">
        ...
        .catch(() => {
            if(grid) {
                grid.innerHTML = `<div style="color:#ff3e3e; font-size:12px; text-align:center; padding:10px;">⚠ Updating live events data, please wait...</div>`;
            }
        });
}
